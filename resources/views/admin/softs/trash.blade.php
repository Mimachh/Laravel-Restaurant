...
@section('content')
    ...
    <table class="user-table">
        ...
        <tbody>
            @foreach($trashedSofts as $soft)
                <tr>
                    <td>{{ $soft->id }}</td>
                    <td>{{ $soft->nom }}</td>
                    <td>{{ $soft->prix }}</td>
                    <td class="checkbox-column">
                        <input type="checkbox" name="selected_items[]" value="{{ $soft->id }}" class="select-checkbox">
                    </td>
                </tr>
            @endforeach
        </tbody>
    ...
    <form id="restoreSelectedForm" action="{{ route('admin.softs.restore-multiple') }}" method="POST" style="display: none;">
        @csrf
        <input type="hidden" name="selectedItems" id="selectedRestoreCheckboxesInput">
    </form>

    <form id="deleteSelectedForm" action="{{ route('admin.softs.force-destroy-multiple') }}" method="POST" style="display: none;">
        @csrf
        @method('DELETE')
