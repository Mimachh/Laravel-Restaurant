...
@section('content')
    ...
    <table class="user-table">
        ...
        <tbody>
            @foreach($trashedAlcools as $alcool)
                <tr>
                    <td>{{ $alcool->id }}</td>
                    <td>{{ $alcool->nom }}</td>
                    <td>{{ $alcool->prix }}</td>
                    <td class="checkbox-column">
                        <input type="checkbox" name="selected_items[]" value="{{ $alcool->id }}" class="select-checkbox">
                    </td>
                </tr>
            @endforeach
        </tbody>
    ...
    <form id="restoreSelectedForm" action="{{ route('admin.alcools.restore-multiple') }}" method="POST" style="display: none;">
        @csrf
        <input type="hidden" name="selectedItems" id="selectedRestoreCheckboxesInput">
    </form>

    <form id="deleteSelectedForm" action="{{ route('admin.alcools.force-destroy-multiple') }}" method="POST" style="display: none;">
        @csrf
        @method('DELETE')
