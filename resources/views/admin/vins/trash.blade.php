...
@section('content')
    ...
    <table class="user-table">
        ...
        <tbody>
            @foreach($trashedVins as $vin)
                <tr>
                    <td>{{ $vin->id }}</td>
                    <td>{{ $vin->name }}</td>
                    <td>{{ $vin->prix }}</td>
                    <td class="checkbox-column">
                        <input type="checkbox" name="selected_items[]" value="{{ $vin->id }}" class="select-checkbox">
                    </td>
                </tr>
            @endforeach
        </tbody>
    ...
    <form id="restoreSelectedForm" action="{{ route('admin.vins.restore-multiple') }}" method="POST" style="display: none;">
        @csrf
        <input type="hidden" name="selectedItems" id="selectedRestoreCheckboxesInput">
    </form>

    <form id="deleteSelectedForm" action="{{ route('admin.vins.force-destroy-multiple') }}" method="POST" style="display: none;">
        @csrf
        @method('DELETE')
