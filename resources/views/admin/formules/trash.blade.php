...
@section('content')
    ...
    <table class="user-table">
        ...
        <tbody>
            @foreach($trashedFormules as $formule)
                <tr>
                    <td>{{ $formule->id }}</td>
                    <td>{{ $formule->nom }}</td>
                    <td>{{ $formule->prix }}</td>
                    <td class="checkbox-column">
                        <input type="checkbox" name="selected_items[]" value="{{ $formule->id }}" class="select-checkbox">
                    </td>
                </tr>
            @endforeach
        </tbody>
    ...
    <form id="restoreSelectedForm" action="{{ route('admin.formules.restore-multiple') }}" method="POST" style="display: none;">
        @csrf
        <input type="hidden" name="selectedItems" id="selectedRestoreCheckboxesInput">
    </form>

    <form id="deleteSelectedForm" action="{{ route('admin.formules.force-destroy-multiple') }}" method="POST" style="display: none;">
        @csrf
        @method('DELETE')
