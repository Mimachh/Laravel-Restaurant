...
@section('content')
            ...
            @foreach($trashedMenus as $menu)
                <tr>
                    ...
                    <td class="checkbox-column">
                        <input type="checkbox" name="selected_items[]" value="{{ $menu->id }}" class="select-checkbox">
                    </td>
                </tr>
            @endforeach
        </tbody>
    ...
    <form id="restoreSelectedForm" action="{{ route('admin.menus.restore-multiple') }}" method="POST" style="display: none;">
        @csrf
        <input type="hidden" name="selectedItems" id="selectedRestoreCheckboxesInput">
    </form>

    <form id="deleteSelectedForm" action="{{ route('admin.menus.force-destroy-multiple') }}" method="POST" style="display: none;">
        @csrf
        @method('DELETE')
